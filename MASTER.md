...
- **Time / calendar layer** — civilizational calendar overlay on journey day-by-day breakdown. Departure day-of-year selector (1-365) with season-aware slider. Each JourneyDay gets `dayOfYear` + active `calendarEvents` from the `calendar.ts` module. Events shown as colour-coded badges in the Days tab. 19 tests. Researched canon events for all 6 civilizations + Basin-wide drawn from `worldbuilder/timeline/calendar/*.yaml` (2026-05-11). ~50 events mapping festivals, harvests, monsoons, political windows, trade seasons, and religious rites to day-of-year.
