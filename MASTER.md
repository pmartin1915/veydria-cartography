...
├── calendar-events.yaml       (56 canon civilizational events)
├── MAP-PROMPT.md
        ...
        └──── web/  (Vite + React 19 + Leaflet + D3 + vitest)
              ...
              └── src/
                  ...
                  └── utils/
                      ...
                      ├── journey-days           per-day breakdown w/ weather + calendar events
...
- **AI Lore panel per feature** — tabbed generative content (Rumours / NPCs / Tensions) for every feature. Mock mode (deterministic seeded placeholders) works without an API key; live mode calls any OpenAI-compatible endpoint. Cached per feature ID + type in `localStorage:veydria.aiLoreCache.v1`. Settings (API key, endpoint, model, temperature) persisted in `veydria.aiLoreSettings.v1`. 45 tests.
